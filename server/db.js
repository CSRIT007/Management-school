...
async function migrateDascPrefixToInv() {
  const { rows: payments } = await pool.query(
    `SELECT id FROM payments WHERE id ~ '^DASC-[0-9]+$' ORDER BY id`
  )
  ...
  const { rows: orders } = await pool.query(
    `SELECT id FROM orders WHERE id ~ '^DASC-[0-9]+$' ORDER BY created_at ASC`
  )
